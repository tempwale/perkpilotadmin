import { useState, type ReactElement, type ReactNode, type KeyboardEvent } from "react";
import DeleteConfirmationPopup from "./DeleteConfirmationPopup";

export interface CardActionPopupProps {
  isOpen: boolean;
  onClose: () => void;
  onEdit?: () => void;
  onDelete?: () => void;
...
}: CardActionPopupProps): ReactElement | null {
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center">
        ...
        <div className="relative w-[min(680px,92%)] p-6 bg-zinc-800 rounded-3xl outline-1 -outline-offset-1 outline-zinc-700 inline-flex flex-col justify-center items-center gap-6 z-10">
          <button
            onClick={(): void => onClose()}
            aria-label="Close popup"
            className="absolute right-4 top-4 text-zinc-300 hover:text-white transition-colors"
          >
            <svg
          ...
          <div className="w-full flex items-center gap-4">
            {icon && <div className="w-12 h-12 shrink-0">{icon}</div>}
          ...
          {/* Preview Component */}
          {previewComponent && <div className="w-full">{previewComponent}</div>}

          {/* Warning Note */}
          <div className="self-stretch flex flex-col justify-start items-center gap-6">
            <div className="self-stretch flex flex-col justify-start items-start gap-4">
              <div className="self-stretch p-3 bg-zinc-700 rounded-2xl inline-flex justify-start items-center gap-2">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="24"
                  height="24"
                ...
                <div className="flex-1 justify-start text-neutral-50 text-sm font-normal font-['Poppins']">
                  Note: This action cannot be undone.
                  {showEditButton && onEdit && ` You can edit this ${entityName} instead of deleting it.`}
                </div>
              </div>
            </div>
            ...
            <div className="self-stretch inline-flex justify-center items-start gap-6">
              {showDeleteButton && (
                <div
                  data-button="ghost"
                  data-size="Large"
                  className="flex-1 h-14 px-10 py-4 rounded-[100px] outline-1 -outline-offset-1 outline-[#ebeef4] flex justify-center items-center gap-2 cursor-pointer hover:bg-zinc-700 transition-colors"
                  ...
                  tabIndex={0}
                >
                  <div className="text-center justify-start text-neutral-50 text-base font-medium font-['Inter'] leading-6">
                    {deleteButtonText}
                  </div>
                </div>
              )}
              ...
              {showEditButton && (
                <div
                  data-button="on"
                  data-size="Large"
                  className="flex-1 px-10 py-4 bg-neutral-50 rounded-[100px] flex justify-center items-center gap-2 cursor-pointer hover:bg-white transition-colors"
                  onClick={(): void => {
                    onEdit?.();
                    onClose();
                  }}
                  ...
                  tabIndex={0}
                >
                  <div className="text-center justify-start text-zinc-950 text-base font-medium font-['Inter'] leading-6">
                    {editButtonText}
                  </div>
                </div>
              )}
      ...
      {confirmDeleteOpen && (
        <DeleteConfirmationPopup
          onClose={(): void => setConfirmDeleteOpen(false)}
          onConfirm={(): void => {
            onDelete?.();
            setConfirmDeleteOpen(false);
            onClose();
          }}
          title={`Want to delete this ${entityName}?`}
          subtitle={`After deleting the ${entityName} it cannot be recovered — this action cannot be undone.`}
          itemId={itemId}
